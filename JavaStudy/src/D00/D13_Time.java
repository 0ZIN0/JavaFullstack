...
public class D13_Time {
	...
	/*		
	  		# java.time 패키지
	  		  ...
	  		  큰 단점이라고 생각하여 새로운 시간 인스턴스를 생성하여 반환한다
	...
	public static void main(String[] args) {
		...
		LocalDate todaySeoul = LocalDate.now(ZoneId.of("America/Cuiaba"));
		
		LocalTime thisTime = LocalTime.now();
		LocalTime thisTimeSeoul = LocalTime.now(ZoneId.of("America/Cuiaba"));
		
		// ZoneId.getAvailableZoneIds() : 사용가능한 ZoneId를 Set으로 반환한다
		System.out.println(ZoneId.getAvailableZoneIds());
		System.out.println(today);
		System.out.println(todaySeoul);
		System.out.println(thisTime);
		System.out.println(thisTimeSeoul);
		...
		dow.getDisplayName(TextStyle.SHORT, Locale.ITALIAN));
		...
		// TemporalAdjster interface : 날짜 조정기능 인터페이스 :
		// TemproalAdjsters : 구현된 날짜 조정기들이 모여있는 클래스
