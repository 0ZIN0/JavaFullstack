...
public class C10_Object {
	...
	/*
	 	...
	 	# equals(obj)
	 	  ...
	 	  '해당 객체로 만들어진 두 인스턴스가 같은지 판정하려면 어떻게 해야하는가'를
	 	  정의 하는 메서드
	 	  매개변수로 전달받은 인스턴스와 this를 비교하여 결과를 리턴한다
	 	  Object 클래스에 정의 되어있는 기본 동작은 해당 인스턴스의 주소값을
	 	  비교하게 되어있다 
	 	
	 	# hash 알고리즘
	 	
	 	  hello! => A12D542F289E078
	 	  hello! => T32QT52T6732190
	 	...
	 	연습 문제 : 적당한 클래스를 하나 설계하고 toString(), equals(), hashCode()를 모두 오버라이드 해보세요
   */
	...
	public static void main(String[] args) {
		...
		Animal prodo = new Animal("프로도", "개", 100);
		Animal popo = new Animal("포포", "개", 70);
		Animal neo = new Animal("네오", "고양이", 80);
		Animal lion = new Animal("라이언", "사자", 150);
		
		System.out.println(prodo.toString());
		System.out.println(prodo.equals(popo));
		System.out.println(prodo.equals(neo));
		
		// 내가 만든적 없지만 Object는 모든 클래스의 부모이므로 
		// toString()같은 Object 메서드들은 반드시 존재할 수 밖에 없다
		String str = p.toString();
		
		// System.out.println()은 전달한 인스턴스의 toString()을 호출해주는 메서드였다
