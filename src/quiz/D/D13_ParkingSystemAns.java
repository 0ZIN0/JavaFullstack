...
public class D13_ParkingSystemAns {
	...
	public static class Car {
		...
		private static String[] types = new String[] {"장애인", "유아동승", "경차", "SUV", "화물차", "승합차", "중장비", "렌트카", "영업용", "법인"};
		private static char[] symbols = new char[] {'가', '나', '다', '라', '마', '바', '사', '아', '자', '하', '허', '호'};
		
		public static String getRandomCarNumber() {
			StringBuilder num = new StringBuilder();
			
			num.append((int)(Math.random() * 990 + 10));
	...
	LocalDate today;
	private static HashMap<String, LocalDate> holidays = new HashMap<>();
	
	public D13_ParkingSystemAns(LocalDate date) {
		today = date;
	...
	public boolean checkParkable(Car car) {
		...
		if(holidays.containsValue(today) || 
			...
			System.out.println("공휴일은 차량5부제가 시행되지 않습니다");
			return false;
		}
		...
		// 평일, 차량 5부제 적용대상 차량만 검사
		if(lastNum == dowValue || lastNum == (dowValue + 5) % 10) {
			System.out.printf("'%s'는 '%s'에 주차할 수 없습니다\n",
	...
	public static void main(String[] args) {
		D13_ParkingSystemAns sys1 = 
				new D13_ParkingSystemAns(LocalDate.now());
		D13_ParkingSystemAns sys2 = 
				new D13_ParkingSystemAns(LocalDate.of(1998, 5, 11));
